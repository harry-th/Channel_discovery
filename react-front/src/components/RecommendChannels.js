...
export default function RecommendChannels() {
  ...
  const testGetSUBS = () => {
    ...
    for (let i = 0; i < addedChannels.length; i++) {
      ...
      axios
        .get(
          `https://youtube.googleapis.com/youtube/v3/subscriptions?part=snippet&channelId=${id}&maxResults=200&key=${API_KEY}`
        )
        .then((data) => {
          Promise.all(
            data.data.items.map((item) => {
            ...
            addedChannels.splice(i, 1, {
              ...oldEntry,
              subscriptions: subsWithStatsandCat,
            });
          });
        })
        .catch(() => {
  ...
  const testGetReccomends = () => {
    ...
    Promise.all(
      channelIds.map((item) => {
        return axios.get(
          `https://youtube.googleapis.com/youtube/v3/channelSections?part=snippet%2CcontentDetails&channelId=${item}&key=${API_KEY}`
        );
      })
    ).then((all) => {
      let x = 0;
      for (let i = 0; i < all.length; i++) {
        let channels = all[i].data.items.find(
          (item) => item.contentDetails?.channels
        ...
        if (channels) {
          ...
          Promise.all(
            channels.map((item) => {
              return axios.get(
                `https://youtube.googleapis.com/youtube/v3/channels?part=snippet&part=statistics&part=topicDetails&id=${item}&key=${API_KEY}`
              );
            })
    ...
    let subscriptions = subs
      ...
      .map((item) => {
        item.subscriptions.map(
          (chnl) => (chnl.snippet.from = item.snippet.title)
        );
